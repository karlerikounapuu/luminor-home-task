class ItemStatusesController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @item_status.update(item_status_params)
        format.html { redirect_to @item_status, notice: "Item status was successfully updated.", status: :see_other }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to item_statuses_path, notice: "Item status was successfully destroyed.", status: :see_other }
  ...
  private
    ...
    def set_item_status
      @item_status = ItemStatus.find(params.expect(:id))
    end

    # Only allow a list of trusted parameters through.
    def item_status_params
      params.expect(item_status: [ :name, :description, :active ])
    end
end
